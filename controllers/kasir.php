...
include '../models/kasir_model.php';
$page = null;
...
switch ($page) {
	...
	case 'list_backup':
		...
		$query = select($where);
		$add_button = "kasir.php?page=list";

		include '../views/kasir/list.php';
		get_footer();
	break;
	...
	case 'list':
		...
		$close_button = "kasir.php?page=list_backup";
		$query_member = select_member();
		$query_pijat = select_pijat();
		$id = (isset($_GET['id'])) ? $_GET['id'] : null;
		if($id){
			$row = read_id($id);
			$action = "kasir.php?page=edit&id=$id";
		...
		$action = "kasir.php?page=save";
	} 
			$row->member_name = false;
			$row->member_alamat = false;
			$row->member_email = false;
			$row->member_phone = false;
		include '../views/kasir/form.php';
		get_footer();
	...
	case 'save':
		...
		header("location: kasir.php?page=list_backup&did=1");	
	break;
	...
	case 'edit':
		...
		header("Location: kasir.php?page=list_backup&id=$id&did=2");
	break;
	...
	case 'delete':
		...
		$id = get_isset($_GET['id']);
		delete($id);
		header('Location: kasir.php?page=list_backup&did=3');
    ...
    $data_s = "'',
    	...
    	var_dump($_POST);
    	// echo $data_s;
    header('location: kasir.php?page=list&id=');
	break;
